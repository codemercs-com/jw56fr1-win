		...
		if (DevInfo != 0)
		{
			...
			if (dHandle != INVALID_HANDLE_VALUE)
			{
				if ((Attributes.VendorID == CODEMERCS_VID) && (Attributes.ProductID == JW56FR1_PID))
				{
					...
					sprintf_s(c_serial, "%ls", serial_buffer);
					...
					if (devCount == 0)
					{
						...
						strcpy_s(serials[itemCount], c_serial);		//Copy serial into list for unique devices
						strcpy_s(m_Device[itemCount].sn, c_serial);	//Copy serial into struct
					}
...
{
	if(m_NumDev > 0)
		strcpy_s(serial, strlen(serial), m_Device[num].sn);
	else
...
void CJoyWarrior56FR1::FillSerial(char* serial, char* res)
{
	...
	strcpy_s(sn, strlen(serial), serial);
	strcpy_s(tmp + (strlen(tmp)-strlen(sn)), (strlen(tmp) - strlen(sn)), serial);
	strcpy_s(res, strlen(tmp), tmp);
}
...
int CJoyWarrior56FR1::Select(char* serial)
{
	...
	if(strlen(serial) < 8)
		...
		strcpy_s(tmp, strlen(tmp), serial);
